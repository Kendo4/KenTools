...
def check_for_fixes(appid: int) -> str:
    ...
    client = ensure_http_client("KenTools: CheckForFixes")
    result = {
        "success": True,
        "appid": appid,
        "gameName": "",
    ...
    try:
        ...
        logger.warn(f"KenTools: Failed to fetch game name for {appid}: {exc}")
        result["gameName"] = f"Unknown Game ({appid})"

    try:
        generic_url = f"https://files.kentools.work/GameBypasses/{appid}.zip"
        ...
        logger.log(f"KenTools: Generic fix check for {appid} -> {resp.status_code}")
    except Exception as exc:
        logger.warn(f"KenTools: Generic fix check failed for {appid}: {exc}")

    try:
        online_url = f"https://files.kentools.work/OnlineFix1/{appid}.zip"
        resp = client.head(online_url, follow_redirects=True, timeout=10)
        logger.log(f"KenTools: Online-fix check ({online_url}) for {appid} -> {resp.status_code}")
        result["onlineFix"]["status"] = resp.status_code
        result["onlineFix"]["available"] = resp.status_code == 200
        if resp.status_code == 200:
            result["onlineFix"]["url"] = online_url
    except Exception as exc:
        logger.warn(f"KenTools: Online-fix check failed for {appid}: {exc}")
...
def _download_and_extract_fix(appid: int, download_url: str, install_path: str, fix_type: str, game_name: str = ""):
    client = ensure_http_client("KenTools: fix download")
    try:
        ...
        logger.log(f"KenTools: Downloading {fix_type} from {download_url}")
            ...
            with open(dest_zip, "wb") as output:
                for chunk in resp.iter_bytes():
                    ...
                    if state.get("status") == "cancelled":
                        logger.log(f"KenTools: Fix download cancelled before writing chunk for {appid}")
                    ...
                    if _get_fix_download_state(appid).get("status") == "cancelled":
                        logger.log(f"KenTools: Fix download cancelled for {appid}")
                        raise RuntimeError("cancelled")

        logger.log(f"KenTools: Download complete, extracting to {install_path}")
        ...
        with zipfile.ZipFile(dest_zip, "r") as archive:
            ...
            if _get_fix_download_state(appid).get("status") == "cancelled":
                logger.log(f"KenTools: Fix extraction cancelled before start for {appid}")
                raise RuntimeError("cancelled")

            if len(top_level_entries) == 1 and appid_folder.rstrip("/") in top_level_entries:
                logger.log(f"KenTools: Found single folder {appid} in zip, extracting its contents")
                for member in archive.namelist():
                    if member.startswith(appid_folder) and member != appid_folder:
                        ...
                        source.close()
                        if _get_fix_download_state(appid).get("status") == "cancelled":
                            logger.log(f"KenTools: Fix extraction cancelled mid-process for {appid}")
                            raise RuntimeError("cancelled")
            else:
                logger.log(f"KenTools: Extracting all zip contents to {install_path}")
                for member in archive.namelist():
                    if member.endswith("/"):
                        continue
        ...
        if _get_fix_download_state(appid).get("status") == "cancelled":
            logger.log(f"KenTools: Fix cancelled after extraction for {appid}")
            raise RuntimeError("cancelled")
                ...
                if ini_relative_path:
                    ...
                    if os.path.exists(ini_full_path):
                        ...
                        if updated_contents != contents:
                            ...
                            logger.log(f"KenTools: Updated unsteam.ini with appid {appid}")
                        else:
                            logger.log("KenTools: unsteam.ini did not contain <appid> placeholder or was already updated")
                    else:
                        logger.warn(f"KenTools: Expected unsteam.ini at {ini_full_path} but file not found")
                else:
                    logger.warn("KenTools: Extracted files do not include unsteam.ini for Online Fix (Unsteam)")
            except Exception as exc:
                logger.warn(f"KenTools: Failed to update unsteam.ini: {exc}")

        log_file_path = os.path.join(install_path, f"kentools-fix-log-{appid}.log")
        try:
            ...
            logger.log(f"KenTools: Appended fix log at {log_file_path} with {len(extracted_files)} files")
        except Exception as exc:
            logger.warn(f"KenTools: Failed to create fix log file: {exc}")

        logger.log(f"KenTools: {fix_type} applied successfully to {install_path}")
        ...
        if str(exc) == "cancelled":
            ...
            return
        logger.warn(f"KenTools: Failed to apply fix: {exc}")
...
def apply_game_fix(appid: int, download_url: str, install_path: str, fix_type: str = "", game_name: str = "") -> str:
    ...
    logger.log(f"KenTools: ApplyGameFix appid={appid}, fixType={fix_type}")
...
def cancel_apply_fix(appid: int) -> str:
    ...
    _set_fix_download_state(appid, {"status": "cancelled", "success": False, "error": "Cancelled by user"})
    logger.log(f"KenTools: CancelApplyFix requested for appid={appid}")
    return json.dumps({"success": True})


def _unfix_game_worker(appid: int, install_path: str, fix_date: str = None):
    try:
        logger.log(f"KenTools: Starting un-fix for appid {appid}, fix_date={fix_date}")
        log_file_path = os.path.join(install_path, f"kentools-fix-log-{appid}.log")
        ...
        try:
            ...
            logger.log(f"KenTools: Found {len(files_to_delete)} unique files to remove from log")
        except Exception as exc:
            logger.warn(f"KenTools: Failed to read log file: {exc}")
        ...
        for file_path in files_to_delete:
            try:
                ...
                if os.path.exists(full_path):
                    os.remove(full_path)
                    deleted_count += 1
                    logger.log(f"KenTools: Deleted {file_path}")
            except Exception as exc:
                logger.warn(f"KenTools: Failed to delete {file_path}: {exc}")

        logger.log(f"KenTools: Deleted {deleted_count}/{len(files_to_delete)} files")
        ...
        if remaining_fixes:
            ...
            try:
                ...
                logger.log(f"KenTools: Updated log file, {len(remaining_fixes)} fixes remaining")
            except Exception as exc:
                logger.warn(f"KenTools: Failed to update log file: {exc}")
        else:
            # No fixes remaining, delete the log file
            try:
                os.remove(log_file_path)
                logger.log(f"KenTools: Deleted log file {log_file_path}")
            except Exception as exc:
                logger.warn(f"KenTools: Failed to delete log file: {exc}")

        _set_unfix_state(appid, {"status": "done", "success": True, "filesRemoved": deleted_count})

    except Exception as exc:
        logger.warn(f"KenTools: Un-fix failed: {exc}")
...
def unfix_game(appid: int, install_path: str = "", fix_date: str = "") -> str:
    ...
    logger.log(f"KenTools: UnFixGame appid={appid}, path={resolved_path}, fix_date={fix_date}")
...
def get_installed_fixes() -> str:
    """Scan all Steam library folders for games with kentools fix logs."""
    try:
        ...
        try:
            ...
            logger.warn(f"KenTools: Failed to parse libraryfolders.vdf: {exc}")
            ...
            try:
                for filename in os.listdir(steamapps_path):
                    ...
                    try:
                        ...
                        # Check for kentools fix log
                        log_file_path = os.path.join(full_install_path, f"kentools-fix-log-{appid}.log")
                        if os.path.exists(log_file_path):
                            ...
                            try:
                                ...
                                logger.warn(f"KenTools: Failed to parse fix log for {appid}: {exc}")

                    except Exception as exc:
                        logger.warn(f"KenTools: Failed to process manifest {filename}: {exc}")
                        continue

            except Exception as exc:
                logger.warn(f"KenTools: Failed to scan library {lib_path}: {exc}")
                continue

        return json.dumps({"success": True, "fixes": installed_fixes})

    except Exception as exc:
        logger.warn(f"KenTools: Failed to get installed fixes: {exc}")
