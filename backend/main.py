...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying KenTools web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy KenTools web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied KenTools icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy KenTools icon: {exc}")
    else:
        logger.warn(f"KenTools icon not found at {icon_src}")


def _inject_webkit_files() -> None:
    js_path = os.path.join(WEBKIT_DIR_NAME, WEB_UI_JS_FILE)
    Millennium.add_browser_js(js_path)
    logger.log(f"KenTools injected web UI: {js_path}")
...
def HasKenToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_kentools_for_app(appid)


def StartAddViaKenTools(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_kentools(appid)


def GetAddViaKenToolsStatus(appid: int, contentScriptQuery: str = "") -> str:
    return get_add_status(appid)


def CancelAddViaKenTools(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_kentools(appid)
...
def DeleteKenToolsForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_kentools_for_app(appid)
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"KenTools: OpenExternalUrl failed: {exc}")
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"KenTools: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "KenTools: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"KenTools: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("KenTools: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("KenTools: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("KenTools: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("KenTools: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
        ...
        if payload is None:
            ...
            logger.warn(f"KenTools: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"KenTools: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"KenTools: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"KenTools: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"KenTools: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"KenTools: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"KenTools: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
class Plugin:
    ...
    def _load(self):
        logger.log(f"bootstrapping KenTools plugin, millennium {Millennium.version()}")

        try:
            detect_steam_install_path()
        except Exception as exc:
            logger.warn(f"KenTools: steam path detection failed: {exc}")
