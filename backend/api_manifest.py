"""Management of the KenTools API manifest (free API list)."""
...
def fetch_free_apis_now(content_script_query: str = "") -> str:
    ...
    client = ensure_http_client("KenTools: FetchFreeApisNow")
    try:
        logger.log("KenTools: FetchFreeApisNow invoked")
        manifest_text = ""
        ...
        try:
            ...
            resp.raise_for_status()
            manifest_text = resp.text
            logger.log("KenTools: Fetched manifest from primary URL")
        except Exception as primary_err:
            logger.warn(f"KenTools: Primary manifest URL failed ({primary_err}), trying proxy...")
            try:
                resp = client.get(
                    API_MANIFEST_PROXY_URL,
                ...
                resp.raise_for_status()
                manifest_text = resp.text
                logger.log("KenTools: Fetched manifest from proxy URL")
            except Exception as proxy_err:
                logger.warn(f"KenTools: Proxy manifest URL also failed: {proxy_err}")
                return json.dumps(
        ...
        logger.warn(f"KenTools: FetchFreeApisNow failed: {exc}")
...
def load_api_manifest() -> List[Dict[str, Any]]:
    ...
    if normalized and normalized != text:
        try:
            ...
            logger.log("KenTools: Normalized api.json to valid JSON")
        except Exception:
            pass
        text = normalized
    ...
    try:
        ...
        logger.error(f"KenTools: Failed to parse api.json: {exc}")
        return []
