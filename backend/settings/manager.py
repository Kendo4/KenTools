...
def _ensure_language_valid(values: Dict[str, Any]) -> bool:
    ...
    if current_language not in available_codes:
        logger.warn(
            f"KenTools: language '{current_language}' not available; "
...
def _ensure_settings_dir() -> None:
    ...
    try:
        ...
        logger.warn(f"KenTools: Failed to ensure settings directory: {exc}")
...
def _load_settings_file() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"KenTools: Failed to read settings file: {exc}")
        return {}
...
def _write_settings_file(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warn(f"KenTools: Failed to persist settings file: {exc}")
...
def _validate_option_value(option: SettingOption, value: Any) -> Tuple[bool, Any, str | None]:
    ...
    if option.option_type == "select":
        ...
        if dynamic == "locales":
            ...
            try:
                logger.log(
                    "KenTools: validating locale option "
            ...
            try:
                logger.warn(
                    f"KenTools: invalid locale selection {value!r}; allowed codes "
            ...
            try:
                logger.warn(
                    f"KenTools: invalid select option value {value!r}; allowed {sorted(allowed)}"
                )
            except Exception:
                pass
            return False, option.default, "Value not in list of allowed options"
...
def apply_settings_changes(changes: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with _SETTINGS_LOCK:
        ...
        for group_key, options_changes in changes.items():
            ...
            logger.log(f"KenTools: applying group {group_key} with payload {options_changes}")
            ...
            for option_key, value in options_changes.items():
                try:
                    logger.log(
                        f"KenTools: apply change request {group_key}.{option_key} -> {value!r}"
                    )
                except Exception:
                    pass
                ...
                try:
                    logger.log(
                        f"KenTools: validated {group_key}.{option_key}, "
        ...
        if not applied_changes and not language_changed:
            ...
            translations = get_locale_manager().get_locale_strings(language)
            logger.log(
                f"KenTools: no changes applied; returning cached values with language={language}"
            )
            return {
                "success": True,
                "values": values_snapshot,
        ...
        for option_key, previous, current_value in applied_changes:
            for callback in _CHANGE_HOOKS.get(option_key, []):
                try:
                    ...
                    logger.warn(f"KenTools: settings hook failed for {option_key}: {exc}")

        translations = get_locale_manager().get_locale_strings(language)

        logger.log(
            f"KenTools: apply_settings_changes final language={language}, values={values_snapshot}"
        )
        return {
            "success": True,
            "values": values_snapshot,
