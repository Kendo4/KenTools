"""Handling of KenTools add/download flows and related utilities."""
...
def _fetch_app_name(appid: int) -> str:
    ...
    client = ensure_http_client("KenTools: _fetch_app_name")
    try:
        ...
        logger.warn(f"KenTools: _fetch_app_name failed for {appid}: {exc}")
...
def _append_loaded_app(appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"KenTools: _append_loaded_app failed for {appid}: {exc}")
...
def _remove_loaded_app(appid: int) -> None:
    try:
        ...
        logger.warn(f"KenTools: _remove_loaded_app failed for {appid}: {exc}")
...
def _log_appid_event(action: str, appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"KenTools: _log_appid_event failed: {exc}")
...
def _preload_app_names_cache() -> None:
    ...
    try:
        ...
        logger.warn(f"KenTools: _preload_app_names_cache from logs failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"KenTools: _preload_app_names_cache from loaded_apps failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"KenTools: _preload_app_names_cache from applist failed: {exc}")
...
def _load_applist_into_memory() -> None:
    ...
    with APPLIST_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("KenTools: Applist file not found, skipping load")
            APPLIST_LOADED = True  # Mark as loaded to avoid repeated checks
            return
        
        try:
            logger.log("KenTools: Loading applist into memory...")
                ...
                for entry in data:
                    if isinstance(entry, dict):
                        ...
                        if appid and name and isinstance(name, str) and name.strip():
                            ...
                            count += 1
                logger.log(f"KenTools: Loaded {count} app names from applist into memory")
            else:
                logger.warn("KenTools: Applist file has invalid format (expected array)")
            
            APPLIST_LOADED = True
        except Exception as exc:
            logger.warn(f"KenTools: Failed to load applist into memory: {exc}")
...
def _ensure_applist_file() -> None:
    ...
    if os.path.exists(file_path):
        logger.log("KenTools: Applist file already exists, skipping download")
        return
    
    logger.log("KenTools: Applist file not found, downloading...")
    client = ensure_http_client("KenTools: DownloadApplist")
    ...
    try:
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warn("KenTools: Downloaded applist has invalid format (expected array)")
                return
        except json.JSONDecodeError as exc:
            logger.warn(f"KenTools: Downloaded applist is not valid JSON: {exc}")
            return
        ...
        logger.log(f"KenTools: Successfully downloaded and saved applist file ({len(data)} entries)")
    except Exception as exc:
        logger.warn(f"KenTools: Failed to download applist file: {exc}")
...
def init_applist() -> None:
    ...
    try:
        ...
        logger.warn(f"KenTools: Applist initialization failed: {exc}")
...
def _process_and_install_lua(appid: int, zip_path: str) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for name in names:
                try:
                    ...
                    if name.lower().endswith(".manifest"):
                        ...
                        logger.log(f"KenTools: Extracted manifest -> {out_path}")
                except Exception as manifest_exc:
                    logger.warn(f"KenTools: Failed to extract manifest {name}: {manifest_exc}")
        except Exception as depot_exc:
            logger.warn(f"KenTools: depotcache extraction failed: {depot_exc}")
        ...
        logger.log(f"KenTools: Installed lua -> {dest_file}")
...
def _download_zip_for_app(appid: int):
    client = ensure_http_client("KenTools: download")
    apis = load_api_manifest()
    if not apis:
        logger.warn("KenTools: No enabled APIs in manifest")
    ...
    for api in apis:
        ...
        logger.log(f"KenTools: Trying API '{name}' -> {url}")
        try:
            headers = {"User-Agent": USER_AGENT}
            if _is_download_cancelled(appid):
                logger.log(f"KenTools: Download cancelled before contacting API '{name}'")
                return
            with client.stream("GET", url, headers=headers, follow_redirects=True) as resp:
                code = resp.status_code
                logger.log(f"KenTools: API '{name}' status={code}")
                ...
                with open(dest_path, "wb") as output:
                    for chunk in resp.iter_bytes():
                        if not chunk:
                            continue
                        if _is_download_cancelled(appid):
                            logger.log(f"KenTools: Download cancelled mid-stream for appid={appid}")
                            raise RuntimeError("cancelled")
                        output.write(chunk)
                        ...
                        if _is_download_cancelled(appid):
                            logger.log(f"KenTools: Download cancelled after writing chunk for appid={appid}")
                            raise RuntimeError("cancelled")
                logger.log(f"KenTools: Download complete -> {dest_path}")

                if _is_download_cancelled(appid):
                    logger.log(f"KenTools: Download marked cancelled after completion for appid={appid}")
                    raise RuntimeError("cancelled")
                ...
                try:
                    with open(dest_path, "rb") as fh:
                        ...
                        if magic not in (b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08"):
                            ...
                            logger.warn(
                                f"KenTools: API '{name}' returned non-zip file (magic={magic.hex()}, size={file_size}, preview={content_preview[:50]})"
                            )
                            try:
                                os.remove(dest_path)
                            except Exception:
                    ...
                    logger.warn("KenTools: Downloaded file not found after download")
                    continue
                except Exception as validation_exc:
                    logger.warn(f"KenTools: File validation failed for API '{name}': {validation_exc}")
                    try:
                        os.remove(dest_path)
                    except Exception:
                        pass
                ...
                try:
                    if _is_download_cancelled(appid):
                        logger.log(f"KenTools: Processing aborted due to cancellation for appid={appid}")
                        raise RuntimeError("cancelled")
                    _set_download_state(appid, {"status": "processing"})
                    _process_and_install_lua(appid, dest_path)
                    if _is_download_cancelled(appid):
                        logger.log(f"KenTools: Installation complete but marked cancelled for appid={appid}")
                        raise RuntimeError("cancelled")
                    try:
                    ...
                    if isinstance(install_exc, RuntimeError) and str(install_exc) == "cancelled":
                        try:
                            ...
                            pass
                        logger.log(f"KenTools: Cancelled download cleanup complete for appid={appid}")
                        return
                    logger.warn(f"KenTools: Processing failed -> {install_exc}")
                    _set_download_state(
            ...
            if str(cancel_exc) == "cancelled":
                try:
                    ...
                    pass
                logger.log(f"KenTools: Download cancelled and cleaned up for appid={appid}")
                return
            logger.warn(f"KenTools: Runtime error during download for appid={appid}: {cancel_exc}")
            _set_download_state(appid, {"status": "failed", "error": str(cancel_exc)})
            return
        except Exception as err:
            logger.warn(f"KenTools: API '{name}' failed with error: {err}")
            continue

    _set_download_state(appid, {"status": "failed", "error": "Not available on any API"})


def start_add_via_kentools(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})

    logger.log(f"KenTools: StartAddViaKenTools appid={appid}")
...
def delete_kentools_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    for path in paths:
        try:
            ...
            logger.warn(f"KenTools: Failed to delete {path}: {exc}")
    try:
...
def get_icon_data_url() -> str:
    try:
        ...
        logger.warn(f"KenTools: GetIconDataUrl failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def has_kentools_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
...
def cancel_add_via_kentools(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    logger.log(f"KenTools: Cancellation requested for appid={appid}")
    return json.dumps({"success": True})
...
def get_installed_lua_scripts() -> str:
    ...
    try:
            ...
            for filename in os.listdir(target_dir):
                ...
                if filename.endswith(".lua") or filename.endswith(".lua.disabled"):
                    try:
                        ...
                        logger.warn(f"KenTools: Failed to process Lua file {filename}: {exc}")
                        continue

        except Exception as exc:
            logger.warn(f"KenTools: Failed to scan stplug-in directory: {exc}")
        ...
        logger.warn(f"KenTools: Failed to get installed Lua scripts: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


__all__ = [
    "cancel_add_via_kentools",
    "delete_kentools_for_app",
    "dismiss_loaded_apps",
    "fetch_app_name",
    "get_add_status",
    "get_icon_data_url",
    "get_installed_lua_scripts",
    "has_kentools_for_app",
    "init_applist",
    "read_loaded_apps",
    "start_add_via_kentools",
]
