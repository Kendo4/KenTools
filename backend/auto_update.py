"""Auto-update utilities for the KenTools backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"KenTools updated to {new_version}. Please restart Steam."
        return "KenTools update applied. Please restart Steam."
...
def _fetch_github_latest(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    asset_name = str(cfg.get("asset_name", "kentoolsplugin.zip")).strip()
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"KenTools updated to {latest_version}. Please restart Steam."
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"KenTools: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("KenTools: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"KenTools: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"KenTools: CheckForUpdatesNow failed: {exc}")
