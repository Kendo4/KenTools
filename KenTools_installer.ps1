...
$KENTOOLS_GITHUB_OWNER = "Kendo4"
$KENTOOLS_GITHUB_REPO = "KenTools"
$KENTOOLS_ASSET_NAME = "kentoolsplugin.zip"
...
function Install-KenTools {
    param([string]$SteamPath)
    
    Write-Info "Installing KenTools..."
    
    $pluginsDir = Join-Path $SteamPath "plugins"
    $kentoolsDir = Join-Path $pluginsDir "kentools"
    ...
    Write-Info "Fetching latest KenTools release from GitHub..."
    $downloadUrl = Get-LatestGitHubRelease -Owner $KENTOOLS_GITHUB_OWNER -Repo $KENTOOLS_GITHUB_REPO -AssetName $KENTOOLS_ASSET_NAME
    ...
    # Download ZIP
    $tempZip = Join-Path $env:TEMP "kentools_temp.zip"
    Write-Info "Downloading KenTools..."
    try {
    ...
    if (Test-Path $kentoolsDir) {
        Write-Info "Removing existing installation..."
        Remove-Item -Path $kentoolsDir -Recurse -Force
    }
    
    # Extract ZIP
    Write-Info "Extracting KenTools..."
    try {
        # Create temporary extraction directory
        $tempExtract = Join-Path $env:TEMP "kentools_extract_temp"
        ...
        foreach ($item in $itemsToCopy) {
            $destPath = Join-Path $kentoolsDir $item.Name
        ...
        Write-Success "KenTools installed successfully!"
        return $true
    } catch {
        Write-Error "Failed to extract KenTools: $_"
...
function Main {
    ...
    # Install KenTools
    Write-Info "Step 2/2: Installing KenTools plugin..."
    $kentoolsResult = Install-KenTools -SteamPath $steamPath
    
    if ($millenniumResult -and $kentoolsResult) {
        Write-Host ""
        Write-ColorOutput "Green" "✓ Installation Complete!"
        Write-Host ""
        Write-Host "Next steps:" -ForegroundColor White
        Write-ColorOutput "Cyan" "1. Restart Steam to load the plugins"
        Write-ColorOutput "Cyan" "2. KenTools should appear in your Steam interface"
        Write-Host ""
        Write-ColorOutput "Red" "Thank you for using KenTools!"
        Write-Host ""
        exit 0
    } else {
        Write-Host ""
        ...
        if (-not $kentoolsResult) {
            Write-Error "KenTools installation failed"
        }
        exit 1
    }
}
