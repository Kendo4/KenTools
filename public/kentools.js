// KenTools button injection (standalone plugin)
(function() {
    'use strict';
    ...
    function backendLog(message) {
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('kentools', 'Logger.log', { message: String(message) });
            }
        } catch (err) {
            if (typeof console !== 'undefined' && console.warn) {
                console.warn('[KenTools] backendLog failed', err);
            }
        }
    }
    
    backendLog('KenTools script loaded');
    // anti-spam state
    ...
    function applyTranslationBundle(bundle) {
        ...
        const stored = window.__KenToolsI18n || {};
        if (bundle.language) {
        ...
        window.__KenToolsI18n = stored;
    }

    function ensureKenToolsStyles() {
        if (document.getElementById('kentools-styles')) return;
        try {
            const style = document.createElement('style');
            style.id = 'kentools-styles';
            style.textContent = `
                .kentools-btn {
                    padding: 12px 24px;
                ...
                .kentools-btn:hover:not([data-disabled="1"]) {
                    background: rgba(220,38,38,0.25);
                    transform: translateY(-2px);
                    box-shadow: 0 6px 20px rgba(220,38,38,0.3);
                    border-color: #dc2626;
                }
                .kentools-btn.primary {
                ...
                .kentools-btn.primary:hover:not([data-disabled="1"]) {
                    background: linear-gradient(135deg, #fca5a5 0%, #dc2626 100%);
                    transform: translateY(-3px) scale(1.03);
                    box-shadow: 0 8px 25px rgba(220,38,38,0.6), inset 0 1px 0 rgba(255,255,255,0.4);
                }
                @keyframes fadeIn {
                    from { opacity: 0; }
                    to { opacity: 1; }
        ...
        } catch(err) { backendLog('KenTools: Styles injection failed: ' + err); }
    }

    function ensureFontAwesome() {
        if (document.getElementById('kentools-fontawesome')) return;
        try {
            const link = document.createElement('link');
            link.id = 'kentools-fontawesome';
            link.rel = 'stylesheet';
        ...
        } catch(err) { backendLog('KenTools: Font Awesome injection failed: ' + err); }
    }

    function showSettingsPopup() {
        if (document.querySelector('.kentools-settings-overlay') || settingsMenuPending) return;
        ...
        ensureTranslationsLoaded(false).catch(function(){ return null; }).finally(function(){
            ...
            if (document.querySelector('.kentools-settings-overlay')) return;

            try { const d = document.querySelector('.kentools-overlay'); if (d) d.remove(); } catch(_) {}
            ensureKenToolsStyles();
            ensureFontAwesome();

            const overlay = document.createElement('div');
            overlay.className = 'kentools-settings-overlay';
            ...
            modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:420px;max-width:600px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(220,38,38,0.3);animation:slideUp 0.1s ease-out;';

            const header = document.createElement('div');
            header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid rgba(220,38,38,0.3);';

            const title = document.createElement('div');
            title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
            title.textContent = t('menu.title', 'KenTools · Menu');
            ...
            function createIconButton(id, iconClass, titleKey, titleFallback) {
                ...
                btn.id = id;
                btn.href = '#';
                btn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(220,38,38,0.1);border:1px solid rgba(220,38,38,0.3);border-radius:10px;color:#dc2626;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
                btn.innerHTML = '<i class="fa-solid ' + iconClass + '"></i>';
                btn.title = t(titleKey, titleFallback);
                btn.onmouseover = function() { this.style.background = 'rgba(220,38,38,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(220,38,38,0.3)'; this.style.borderColor = '#dc2626'; };
                btn.onmouseout = function() { this.style.background = 'rgba(220,38,38,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(220,38,38,0.3)'; };
            ...
            function createSectionLabel(key, fallback, marginTop) {
                ...
                label.style.cssText = 'font-size:12px;color:#dc2626;margin-top:' + topValue + 'px;margin-bottom:4px;font-weight:600;text-transform:uppercase;letter-spacing:1.2px;text-align:center;';
            ...
            function createMenuButton(id, key, fallback, iconClass, isPrimary) {
                ...
                btn.id = id;
                btn.href = '#';
                btn.style.cssText = 'display:flex;align-items:center;justify-content:center;gap:8px;padding:14px 24px;background:linear-gradient(135deg, rgba(220,38,38,0.15) 0%, rgba(220,38,38,0.05) 100%);border:1px solid rgba(220,38,38,0.3);border-radius:12px;color:#fff;font-size:15px;font-weight:500;text-decoration:none;transition:all 0.3s ease;cursor:pointer;position:relative;overflow:hidden;text-align:center;';
                ...
                btn.onmouseover = function() { this.style.background = 'linear-gradient(135deg, rgba(220,38,38,0.3) 0%, rgba(220,38,38,0.15) 100%)'; this.style.transform = 'translateY(-2px)'; this.style.boxShadow = '0 8px 20px rgba(220,38,38,0.25)'; this.style.borderColor = '#dc2626'; };
                btn.onmouseout = function() { this.style.background = 'linear-gradient(135deg, rgba(220,38,38,0.15) 0%, rgba(220,38,38,0.05) 100%)'; this.style.transform = 'translateY(0)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(220,38,38,0.3)'; };
            ...
            const removeBtn = createMenuButton('lt-settings-remove-lua', 'menu.removeKenTools', 'Remove via KenTools', 'fa-trash-can');
            ...
            if (checkBtn) {
                checkBtn.addEventListener('click', function(e){
                    ...
                    try {
                        Millennium.callServerMethod('kentools', 'CheckForUpdatesNow', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                const msg = (payload && payload.message) ? String(payload.message) : lt('No updates available.');
                                ShowKenToolsAlert('KenTools', msg);
                            } catch(_) {}
                        });
                    } catch(_) {}
                });
            ...
            if (discordBtn) {
                discordBtn.addEventListener('click', function(e){
                    e.preventDefault();
                    try { overlay.remove(); } catch(_) {}
                    const url = 'https://dsc.gg/kentools';
                    try {
                        Millennium.callServerMethod('kentools', 'OpenExternalUrl', { url, contentScriptQuery: '' });
                    } catch(_) {}
                });
            }
            ...
            if (fetchApisBtn) {
                fetchApisBtn.addEventListener('click', function(e){
                    ...
                    try { overlay.remove(); } catch(_) {}
                    try {
                        Millennium.callServerMethod('kentools', 'FetchFreeApisNow', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                ...
                                ShowKenToolsAlert('KenTools', text);
                            } catch(_) {}
                        });
                    } catch(_) {}
                });
            ...
            if (fixesMenuBtn) {
                fixesMenuBtn.addEventListener('click', function(e){
                    ...
                    try {
                        ...
                        const appid = match ? parseInt(match[1], 10) : (window.__KenToolsCurrentAppId || NaN);
                        if (isNaN(appid)) {
                            try { overlay.remove(); } catch(_) {}
                            const errText = t('menu.error.noAppId', 'Could not determine game AppID');
                            ShowKenToolsAlert('KenTools', errText);
                            return;
                        }

                        Millennium.callServerMethod('kentools', 'GetGameInstallPath', { appid, contentScriptQuery: '' }).then(function(pathRes){
                            try {
                                ...
                                if (pathPayload && pathPayload.success && pathPayload.installPath) {
                                    ...
                                    window.__KenToolsGameInstallPath = pathPayload.installPath;
                                }
                                window.__KenToolsGameIsInstalled = isGameInstalled;
                                try { overlay.remove(); } catch(_) {}
                                showFixesLoadingPopupAndCheck(appid);
                            } catch(err) {
                                backendLog('KenTools: GetGameInstallPath error: ' + err);
                                try { overlay.remove(); } catch(_) {}
                            }
                        }).catch(function() {
                            try { overlay.remove(); } catch(_) {}
                            const errorText = t('menu.error.getPath', 'Error getting game path');
                            ShowKenToolsAlert('KenTools', errorText);
                        });
                    } catch(err) {
                        backendLog('KenTools: Fixes Menu button error: ' + err);
                    }
                });
            }
            ...
            try {
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('kentools', 'HasKenToolsForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            ...
                            if (exists) {
                                const doDelete = function() {
                                    try {
                                        Millennium.callServerMethod('kentools', 'DeleteKenToolsForApp', { appid, contentScriptQuery: '' }).then(function(){
                                            try {
                                                window.__KenToolsButtonInserted = false;
                                                window.__KenToolsPresenceCheckInFlight = false;
                                                window.__KenToolsPresenceCheckAppId = undefined;
                                                addKenToolsButton();
                                                const successText = t('menu.remove.success', 'KenTools removed for this app.');
                                                ShowKenToolsAlert('KenTools', successText);
                                            } catch(err) {
                                                backendLog('KenTools: post-delete cleanup failed: ' + err);
                                            }
                                        }).catch(function(err){
                                            const failureText = t('menu.remove.failure', 'Failed to remove KenTools.');
                                            const errMsg = (err && err.message) ? err.message : failureText;
                                            ShowKenToolsAlert('KenTools', errMsg);
                                        });
                                    } catch(err) {
                                        backendLog('KenTools: doDelete failed: ' + err);
                                    }
                                };
                                ...
                                removeBtn.onclick = function(e){
                                    e.preventDefault();
                                    try { overlay.remove(); } catch(_) {}
                                    const confirmMessage = t('menu.remove.confirm', 'Remove via KenTools for this game?');
                                    showKenToolsConfirm('KenTools', confirmMessage, function(){
                                        doDelete();
                                    }, function(){
    ...
    function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
        try {
            if (!forceRefresh && window.__KenToolsI18n && window.__KenToolsI18n.ready) {
                return Promise.resolve(window.__KenToolsI18n);
            }
            if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
                window.__KenToolsI18n = window.__KenToolsI18n || { language: 'en', locales: [], strings: {}, ready: false };
                return Promise.resolve(window.__KenToolsI18n);
            }
            const targetLanguage = (typeof preferredLanguage === 'string' && preferredLanguage) ? preferredLanguage :
                ((window.__KenToolsI18n && window.__KenToolsI18n.language) || '');
            return Millennium.callServerMethod('kentools', 'GetTranslations', { language: targetLanguage, contentScriptQuery: '' }).then(function(res){
                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                ...
                return window.__KenToolsI18n;
            }).catch(function(err){
                backendLog('KenTools: translation load failed: ' + err);
                window.__KenToolsI18n = window.__KenToolsI18n || { language: 'en', locales: [], strings: {}, ready: false };
                return window.__KenToolsI18n;
            });
        } catch(err) {
            backendLog('KenTools: ensureTranslationsLoaded error: ' + err);
            window.__KenToolsI18n = window.__KenToolsI18n || { language: 'en', locales: [], strings: {}, ready: false };
            return Promise.resolve(window.__KenToolsI18n);
        }
    }
    ...
    function translateText(key, fallback) {
        ...
        try {
            const store = window.__KenToolsI18n;
    ...
    function showTestPopup() {
        ...
        // Avoid duplicates
        if (document.querySelector('.kentools-overlay')) return;
        // Close settings popup if open so modals don't overlap
        try { const s = document.querySelector('.kentools-settings-overlay'); if (s) s.remove(); } catch(_) {}
        
        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:400px;max-width:560px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(220,38,38,0.3);animation:slideUp 0.1s ease-out;';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:22px;color:#fff;margin-bottom:16px;font-weight:700;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.className = 'kentools-title';
        title.textContent = 'KenTools';

        const body = document.createElement('div');
        body.style.cssText = 'font-size:14px;line-height:1.4;margin-bottom:12px;';
        body.className = 'kentools-status';
        body.textContent = lt('Working…');

        const progressWrap = document.createElement('div');
        progressWrap.style.cssText = 'background:rgba(26,26,26,0.5);height:12px;border-radius:4px;overflow:hidden;position:relative;display:none;border:1px solid rgba(220,38,38,0.3);';
        progressWrap.className = 'kentools-progress-wrap';
        const progressBar = document.createElement('div');
        progressBar.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg, #dc2626 0%, #f87171 100%);transition:width 0.1s linear;box-shadow:0 0 10px rgba(220,38,38,0.5);';
        progressBar.className = 'kentools-progress-bar';
        progressWrap.appendChild(progressBar);

        const percent = document.createElement('div');
        percent.style.cssText = 'text-align:right;color:#8f98a0;margin-top:8px;font-size:12px;display:none;';
        percent.className = 'kentools-percent';
        percent.textContent = '0%';

        const btnRow = document.createElement('div');
        btnRow.style.cssText = 'margin-top:16px;display:flex;gap:8px;justify-content:flex-end;';
        const cancelBtn = document.createElement('a');
        cancelBtn.className = 'btnv6_blue_hoverfade btn_medium kentools-cancel-btn';
        ...
        hideBtn.className = 'btnv6_blue_hoverfade btn_medium kentools-hide-btn';
        ...
        function cancelOperation(){
            ...
            try {
                ...
                const appid = match ? parseInt(match[1], 10) : (window.__KenToolsCurrentAppId || NaN);
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('kentools', 'CancelAddViaKenTools', { appid, contentScriptQuery: '' });
                }
            } catch(_) {}
            // Update UI to show cancelled
            const status = overlay.querySelector('.kentools-status');
            if (status) status.textContent = lt('Cancelled');
            const cancelBtn = overlay.querySelector('.kentools-cancel-btn');
            if (cancelBtn) cancelBtn.style.display = 'none';
            const hideBtn = overlay.querySelector('.kentools-hide-btn');
            if (hideBtn) hideBtn.innerHTML = `<span>${lt('Close')}</span>`;
            // Hide progress UI
            const wrap = overlay.querySelector('.kentools-progress-wrap');
            const percent = overlay.querySelector('.kentools-percent');
    ...
    function showFixesResultsPopup(data, isGameInstalled) {
        if (document.querySelector('.kentools-fixes-results-overlay')) return;
        // Close other popups
        try { const d = document.querySelector('.kentools-overlay'); if (d) d.remove(); } catch(_) {}
        try { const s = document.querySelector('.kentools-settings-overlay'); if (s) s.remove(); } catch(_) {}
        try { const f = document.querySelector('.kentools-fixes-results-overlay'); if (f) f.remove(); } catch(_) {}
        try { const l = document.querySelector('.kentools-loading-fixes-overlay'); if (l) l.remove(); } catch(_) {}

        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-fixes-results-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';

        const modal = document.createElement('div');
        modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:580px;max-width:700px;max-height:80vh;display:flex;flex-direction:column;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(220,38,38,0.3);animation:slideUp 0.1s ease-out;';

        const header = document.createElement('div');
        header.style.cssText = 'flex:0 0 auto;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid rgba(220,38,38,0.3);';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.textContent = lt('KenTools · Fixes Menu');
        ...
        const body = document.createElement('div');
        body.style.cssText = 'flex:1 1 auto;overflow-y:auto;padding:20px;border:1px solid rgba(220,38,38,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        ...
        function createFixButton(label, text, icon, isSuccess, onClick) {
            ...
            sectionLabel.style.cssText = 'font-size:12px;color:#dc2626;margin-bottom:8px;font-weight:600;text-transform:uppercase;letter-spacing:1px;';
            sectionLabel.textContent = label;

            const btn = document.createElement('a');
            btn.href = '#';
            btn.style.cssText = 'display:flex;align-items:center;justify-content:center;gap:10px;width:100%;box-sizing:border-box;padding:14px 24px;background:linear-gradient(135deg, rgba(220,38,38,0.15) 0%, rgba(220,38,38,0.05) 100%);border:1px solid rgba(220,38,38,0.3);border-radius:12px;color:#fff;font-size:15px;font-weight:500;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        ...
        const genericSection = createFixButton(
            ...
            function(e) {
                ...
                if (genericStatus === 200 && isGameInstalled) {
                    const genericUrl = 'https://files.kentools.work/GameBypasses/' + data.appid + '.zip';
        ...
        const onlineSection = createFixButton(
            ...
            function(e) {
                ...
                if (onlineStatus === 200 && isGameInstalled) {
                    const onlineUrl = data.onlineFix.url || ('https://files.kentools.work/OnlineFix1/' + data.appid + '.zip');
        ...
        const unfixSection = createFixButton(
            ...
            function(e) {
                ...
                if (isGameInstalled) {
                    try { overlay.remove(); } catch(_) {}
                    showKenToolsConfirm('KenTools', lt('Are you sure you want to un-fix? This will remove fix files and verify game files.'),
        ...
        gameFolderBtn.onclick = function(e){ 
            e.preventDefault(); 
            if (window.__KenToolsGameInstallPath) {
                try {
                    Millennium.callServerMethod('kentools', 'OpenGameFolder', { path: window.__KenToolsGameInstallPath, contentScriptQuery: '' });
                } catch(err) { backendLog('KenTools: Failed to open game folder: ' + err); }
            }
        };
        ...
        discordBtn.onclick = function(e) {
            ...
            try { overlay.remove(); } catch(_) {}
            const url = 'https://dsc.gg/kentools';
            try { Millennium.callServerMethod('kentools', 'OpenExternalUrl', { url, contentScriptQuery: '' }); } catch(_) {}
        };
        ...
        function startUnfix(appid) {
            try {
                Millennium.callServerMethod('kentools', 'UnFixGame', { appid: appid, installPath: window.__KenToolsGameInstallPath, contentScriptQuery: '' }).then(function(res){
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        ...
                        ShowKenToolsAlert('KenTools', errorMsg);
                    }
                }).catch(function(){
                    const msg = lt('Error starting un-fix');
                    ShowKenToolsAlert('KenTools', msg);
                });
            } catch(err) { backendLog('KenTools: Un-Fix start error: ' + err); }
        }
    }

    function showFixesLoadingPopupAndCheck(appid) {
        if (document.querySelector('.kentools-loading-fixes-overlay')) return;
        try { const d = document.querySelector('.kentools-overlay'); if (d) d.remove(); } catch(_) {}
        try { const s = document.querySelector('.kentools-settings-overlay'); if (s) s.remove(); } catch(_) {}
        try { const f = document.querySelector('.kentools-fixes-overlay'); if (f) f.remove(); } catch(_) {}

        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-loading-fixes-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        const progressWrap = document.createElement('div');
        progressWrap.style.cssText = 'background:rgba(26,26,26,0.5);height:12px;border-radius:4px;overflow:hidden;position:relative;border:1px solid rgba(220,38,38,0.3);';
        const progressBar = document.createElement('div');
        progressBar.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg, #dc2626 0%, #f87171 100%);transition:width 0.2s linear;box-shadow:0 0 10px rgba(220,38,38,0.5);';
        progressWrap.appendChild(progressBar);
        ...
        Millennium.callServerMethod('kentools', 'CheckForFixes', { appid, contentScriptQuery: '' }).then(function(res){
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (payload && payload.success) {
                const isGameInstalled = window.__KenToolsGameIsInstalled === true;
            ...
            setTimeout(function() {
                try {
                    const l = document.querySelector('.kentools-loading-fixes-overlay');
                    if (l) l.remove();
                } catch(_) {}
            }, 300);
        });
    ...
    function applyFix(appid, downloadUrl, fixType, gameName, resultsOverlay) {
        try {
            ...
            backendLog('KenTools: Applying fix ' + fixType + ' for appid ' + appid);
            
            // Start the download and extraction process
            Millennium.callServerMethod('kentools', 'ApplyGameFix', { 
                appid: appid, 
                downloadUrl: downloadUrl, 
                installPath: window.__KenToolsGameInstallPath,
                fixType: fixType,
                gameName: gameName || '',
                contentScriptQuery: '' 
            }).then(function(res){
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        // Show download progress popup similar to Add via KenTools
                    ...
                    backendLog('KenTools: ApplyGameFix response error: ' + err);
                    const msg = lt('Error applying fix');
                    ShowKenToolsAlert('KenTools', msg);
                }
            }).catch(function(err){
                backendLog('KenTools: ApplyGameFix error: ' + err);
                const msg = lt('Error applying fix');
                ShowKenToolsAlert('KenTools', msg);
            });
        } catch(err) {
            backendLog('KenTools: applyFix error: ' + err);
        }
    }

    // Show fix download progress popup
    function showFixDownloadProgress(appid, fixType) {
        // Reuse the download popup UI from Add via KenTools
        if (document.querySelector('.kentools-overlay')) return;
        ...
        const hideBtn = document.createElement('a');
        hideBtn.href = '#';
        hideBtn.className = 'kentools-btn';
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'kentools-btn primary';
        ...
        cancelBtn.onclick = function(e){
            ...
            Millennium.callServerMethod('kentools', 'CancelApplyFix', { appid: appid, contentScriptQuery: '' }).then(function(res){
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    ...
                    backendLog('KenTools: CancelApplyFix response error: ' + err);
                ...
                const msgEl2 = document.getElementById('lt-fix-progress-msg');
                if (msgEl2 && msgEl2.dataset.last) msgEl2.textContent = msgEl2.dataset.last;
                backendLog('KenTools: CancelApplyFix error: ' + err);
                const msg = lt('Failed to cancel fix download');
                ShowKenToolsAlert('KenTools', msg);
            });
        };
    ...
    function replaceFixButtonsWithClose(overlayEl) {
        ...
        closeBtn.href = '#';
        closeBtn.className = 'kentools-btn primary';
    ...
    function pollFixProgress(appid, fixType) {
        const poll = function() {
            try {
                const overlayEl = document.querySelector('.kentools-overlay');
                if (!overlayEl) return; // Stop if overlay was closed
                
                Millennium.callServerMethod('kentools', 'GetApplyFixStatus', { appid: appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        ...
                        backendLog('KenTools: GetApplyFixStatus error: ' + err);
                    }
                });
            } catch(err) {
                backendLog('KenTools: pollFixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function showUnfixProgress(appid) {
        ...
        try { const old = document.querySelector('.kentools-unfix-overlay'); if (old) old.remove(); } catch(_) {}

        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-unfix-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
    ...
    function pollUnfixProgress(appid) {
        const poll = function() {
            try {
                const overlayEl = document.querySelector('.kentools-unfix-overlay');
                if (!overlayEl) return; // Stop if overlay was closed
                
                Millennium.callServerMethod('kentools', 'GetUnfixStatus', { appid: appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.success && payload.state) {
                            const state = payload.state;
                            ...
                            if (state.status === 'removing') {
                                ...
                                setTimeout(function(){
                                    try {
                                        ...
                                        backendLog('KenTools: Running verify for appid ' + appid);
                                    } catch(_) {}
                                }, 1000);
                        ...
                        backendLog('KenTools: GetUnfixStatus error: ' + err);
                    }
                });
            } catch(err) {
                backendLog('KenTools: pollUnfixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function fetchSettingsConfig(forceRefresh) {
        try {
            if (!forceRefresh && window.__KenToolsSettings && Array.isArray(window.__KenToolsSettings.schema)) {
                return Promise.resolve(window.__KenToolsSettings);
            }
        } catch(_) {}

        if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
            return Promise.reject(new Error(lt('KenTools backend unavailable')));
        }

        return Millennium.callServerMethod('kentools', 'GetSettingsConfig', { contentScriptQuery: '' }).then(function(res){
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (!payload || payload.success !== true) {
            ...
            window.__KenToolsSettings = config;
            return config;
        });
    }
    ...
    function showSettingsManagerPopup(forceRefresh, onBack) {
        if (document.querySelector('.kentools-settings-manager-overlay')) return;

        try { const mainOverlay = document.querySelector('.kentools-settings-overlay'); if (mainOverlay) mainOverlay.remove(); } catch(_) {}

        ensureKenToolsStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'kentools-settings-manager-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'position:relative;background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:650px;max-width:750px;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(220,38,38,0.3);animation:slideUp 0.1s ease-out;overflow:hidden;';

        const header = document.createElement('div');
        header.style.cssText = 'display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding:28px 32px 16px;border-bottom:2px solid rgba(220,38,38,0.3);';

        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;font-weight:700;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        title.textContent = t('settings.title', 'KenTools · Settings');
        ...
        discordIconBtn.href = '#';
        discordIconBtn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(220,38,38,0.1);border:1px solid rgba(220,38,38,0.3);border-radius:10px;color:#dc2626;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        discordIconBtn.innerHTML = '<i class="fa-brands fa-discord"></i>';
        discordIconBtn.title = t('menu.discord', 'Discord');
        discordIconBtn.onmouseover = function() { this.style.background = 'rgba(220,38,38,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(220,38,38,0.3)'; this.style.borderColor = '#dc2626'; };
        discordIconBtn.onmouseout = function() { this.style.background = 'rgba(220,38,38,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(220,38,38,0.3)'; };
        iconButtons.appendChild(discordIconBtn);

        const closeIconBtn = document.createElement('a');
        closeIconBtn.href = '#';
        closeIconBtn.style.cssText = 'display:flex;align-items:center;justify-content:center;width:40px;height:40px;background:rgba(220,38,38,0.1);border:1px solid rgba(220,38,38,0.3);border-radius:10px;color:#dc2626;font-size:18px;text-decoration:none;transition:all 0.3s ease;cursor:pointer;';
        closeIconBtn.innerHTML = '<i class="fa-solid fa-xmark"></i>';
        closeIconBtn.title = t('settings.close', 'Close');
        closeIconBtn.onmouseover = function() { this.style.background = 'rgba(220,38,38,0.25)'; this.style.transform = 'translateY(-2px) scale(1.05)'; this.style.boxShadow = '0 8px 16px rgba(220,38,38,0.3)'; this.style.borderColor = '#dc2626'; };
        closeIconBtn.onmouseout = function() { this.style.background = 'rgba(220,38,38,0.1)'; this.style.transform = 'translateY(0) scale(1)'; this.style.boxShadow = 'none'; this.style.borderColor = 'rgba(220,38,38,0.3)'; };
        iconButtons.appendChild(closeIconBtn);

        const contentWrap = document.createElement('div');
        contentWrap.style.cssText = 'flex:1 1 auto;overflow-y:auto;overflow-x:hidden;padding:20px;margin:0 24px;border:1px solid rgba(220,38,38,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        ...
        function setStatus(text, color) {
            let statusLine = contentWrap.querySelector('.kentools-settings-status');
            if (!statusLine) {
                statusLine = document.createElement('div');
                statusLine.className = 'kentools-settings-status';
        ...
        function renderSettings() {
            ...
            for (let i = 0; i < state.config.schema.length; i++) {
                ...
                if (group.key === 'general') {
                    ...
                    groupTitle.style.cssText = 'font-size:15px;font-weight:600;color:#dc2626;text-align:center;';
                }
                ...
                for (let j = 0; j < options.length; j++) {
                    ...
                    if (j === 0) {
                        ...
                        optionEl.style.cssText = 'margin-top:12px;padding-top:12px;border-top:1px solid rgba(220,38,38,0.1);';
                    }
                    ...
                    if (option.type === 'select') {
                        ...
                        selectEl.addEventListener('change', function(){
                            ...
                            try { backendLog('KenTools: language select changed to ' + selectEl.value); } catch(_) {}
                            updateSaveState();
        ...
        function renderInstalledFixesSection() {
            ...
            sectionEl.id = 'kentools-installed-fixes-section';
            sectionEl.style.cssText = 'margin-top:36px;padding:24px;background:linear-gradient(135deg, rgba(220,38,38,0.05) 0%, rgba(74,158,206,0.08) 100%);border:2px solid rgba(74,158,206,0.3);border-radius:14px;box-shadow:0 4px 15px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05);position:relative;overflow:hidden;';

            const sectionGlow = document.createElement('div');
            sectionGlow.style.cssText = 'position:absolute;top:-100%;left:-100%;width:300%;height:300%;background:radial-gradient(circle, rgba(220,38,38,0.08) 0%, transparent 70%);pointer-events:none;';
            sectionEl.appendChild(sectionGlow);

            const sectionTitle = document.createElement('div');
            sectionTitle.style.cssText = 'font-size:22px;color:#dc2626;margin-bottom:20px;font-weight:700;text-align:center;text-shadow:0 2px 10px rgba(220,38,38,0.5);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;position:relative;z-index:1;letter-spacing:0.5px;';
            ...
            listContainer.id = 'kentools-fixes-list';
        ...
        function loadInstalledFixes(container) {
            ...
            Millennium.callServerMethod('kentools', 'GetInstalledFixes', { contentScriptQuery: '' })
                .then(function(res) {
        ...
        function createFixListItem(fix, container) {
            ...
            itemEl.style.cssText = 'margin-bottom:12px;padding:14px;background:rgba(11,20,30,0.8);border:1px solid rgba(220,38,38,0.3);border-radius:6px;display:flex;justify-content:space-between;align-items:center;transition:all 0.2s ease;';
            itemEl.onmouseover = function() { this.style.borderColor = '#dc2626'; this.style.background = 'rgba(11,20,30,0.95)'; };
            itemEl.onmouseout = function() { this.style.borderColor = 'rgba(220,38,38,0.3)'; this.style.background = 'rgba(11,20,30,0.8)'; };
            ...
            if (fix.fixType) {
                ...
                typeSpan.innerHTML = '<strong style="color:#dc2626;">' + t('settings.installedFixes.type', 'Type:') + '</strong> ' + fix.fixType;
                detailsDiv.appendChild(typeSpan);
            }

            if (fix.date) {
                const dateSpan = document.createElement('div');
                dateSpan.innerHTML = '<strong style="color:#dc2626;">' + t('settings.installedFixes.date', 'Installed:') + '</strong> ' + fix.date;
                detailsDiv.appendChild(dateSpan);
            }

            if (fix.filesCount > 0) {
                const filesSpan = document.createElement('div');
                filesSpan.innerHTML = '<strong style="color:#dc2626;">' + t('settings.installedFixes.files', '{count} files').replace('{count}', fix.filesCount) + '</strong>';
            ...
            deleteBtn.addEventListener('click', function(e) {
                ...
                showKenToolsConfirm(
                    ...
                    function() {
                        ...
                        Millennium.callServerMethod('kentools', 'UnFixGame', {
                            appid: fix.appid,
        ...
        function pollUnfixStatus(appid, itemEl, deleteBtn, container) {
            ...
            function checkStatus() {
                ...
                Millennium.callServerMethod('kentools', 'GetUnfixStatus', { appid: appid, contentScriptQuery: '' })
                    .then(function(res) {
                        const response = typeof res === 'string' ? JSON.parse(res) : res;
                        if (!response || !response.success) {
        ...
        function renderInstalledLuaSection() {
            const sectionEl = document.createElement('div');
            sectionEl.id = 'kentools-installed-lua-section';
            ...
            const listContainer = document.createElement('div');
            listContainer.id = 'kentools-lua-list';
            listContainer.style.cssText = 'min-height:50px;';
            sectionEl.appendChild(listContainer);
        ...
        function loadInstalledLuaScripts(container) {
            ...
            Millennium.callServerMethod('kentools', 'GetInstalledLuaScripts', { contentScriptQuery: '' })
                .then(function(res) {
                    const response = typeof res === 'string' ? JSON.parse(res) : res;
                    if (!response || !response.success) {
                    ...
                    if (hasUnknownGames) {
                        ...
                        infoBanner.innerHTML = '<i class="fa-solid fa-circle-info" style="font-size:16px;"></i><span>' + t('settings.installedLua.unknownInfo', 'Games showing \'Unknown Game\' were installed manually (not via KenTools).') + '</span>';
        ...
        function createLuaListItem(script, container) {
            ...
            if (script.modifiedDate) {
                const dateSpan = document.createElement('div');
                dateSpan.innerHTML = '<strong style="color:#dc2626;">' + t('settings.installedLua.modified', 'Modified:') + '</strong> ' + script.modifiedDate;
                detailsDiv.appendChild(dateSpan);
            }
            ...
            deleteBtn.addEventListener('click', function(e) {
                ...
                showKenToolsConfirm(
                    script.gameName || 'KenTools',
                    t('settings.installedLua.deleteConfirm', 'Remove via KenTools for this game?'),
                    function() {
                        // User confirmed
                        deleteBtn.dataset.busy = '1';
                        deleteBtn.style.opacity = '0.6';
                        ...
                        Millennium.callServerMethod('kentools', 'DeleteKenToolsForApp', {
                            appid: script.appid,
                            contentScriptQuery: ''
                        })
                        .then(function(res) {
        ...
        saveBtn.addEventListener('click', function(e){
            ...
            try { backendLog('KenTools: collectChanges payload ' + JSON.stringify(changes)); } catch(_) {}
            ...
            try { backendLog('KenTools: sending settings payload ' + JSON.stringify(payloadToSend)); } catch(_) {}
            // Pass flattened keys so Millennium handles the RPC arguments as expected.
            Millennium.callServerMethod('kentools', 'ApplySettingsChanges', {
                contentScriptQuery: '',
                ...
                try {
                    if (window.__KenToolsSettings) {
                        window.__KenToolsSettings.values = JSON.parse(JSON.stringify(state.config.values));
                        window.__KenToolsSettings.schemaVersion = state.config.schemaVersion;
                        window.__KenToolsSettings.lastFetched = Date.now();
                        if (response && response.translations && typeof response.translations === 'object') {
                            window.__KenToolsSettings.translations = response.translations;
                        }
                        if (response && response.language) {
                            window.__KenToolsSettings.language = response.language;
                        }
                    }
                } catch(_) {}
                ...
                if (response && response.translations && typeof response.translations === 'object') {
                    applyTranslationBundle({
                        language: response.language || (window.__KenToolsI18n && window.__KenToolsI18n.language) || 'en',
                        locales: (window.__KenToolsI18n && window.__KenToolsI18n.locales) || (state.config && state.config.locales) || [],
    ...
    function closeSettingsOverlay() {
        try {
            ...
            var list = document.getElementsByClassName('kentools-settings-overlay');
            while (list && list.length > 0) {
                try { list[0].remove(); } catch(_) { break; }
            }
            // Also remove any download/progress overlays if present
            var list2 = document.getElementsByClassName('kentools-overlay');
    ...
    function showKenToolsAlert(title, message, onClose) {
        if (document.querySelector('.kentools-alert-overlay')) return;

        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-alert-overlay';
        ...
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:400px;max-width:520px;padding:32px 36px;box-shadow:0 20px 60px rgba(0,0,0,.9), 0 0 0 1px rgba(220,38,38,0.4);animation:slideUp 0.1s ease-out;';

        const titleEl = document.createElement('div');
        titleEl.style.cssText = 'font-size:22px;color:#fff;margin-bottom:20px;font-weight:700;text-align:left;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        titleEl.textContent = String(title || 'KenTools');
        ...
        okBtn.href = '#';
        okBtn.className = 'kentools-btn primary';
    ...
    function ShowKenToolsAlert(title, message) {
        try {
            showKenToolsAlert(title, message);
        } catch(err) {
            backendLog('KenTools: Alert error, falling back: ' + err);
            try { alert(String(title) + '\n\n' + String(message)); } catch(_) {}
        }
    }

    // Steam-style confirm helper (ShowConfirmDialog only)
    function showKenToolsConfirm(title, message, onConfirm, onCancel) {
        // Always close settings popup first so the confirm is visible on top
        closeSettingsOverlay();

        // Create custom modern confirmation dialog
        if (document.querySelector('.kentools-confirm-overlay')) return;

        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.className = 'kentools-confirm-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';

        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:420px;max-width:540px;padding:32px 36px;box-shadow:0 20px 60px rgba(0,0,0,.9), 0 0 0 1px rgba(220,38,38,0.4);animation:slideUp 0.1s ease-out;';

        const titleEl = document.createElement('div');
        titleEl.style.cssText = 'font-size:22px;color:#fff;margin-bottom:20px;font-weight:700;text-align:center;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;';
        titleEl.textContent = String(title || 'KenTools');
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'kentools-btn';
        cancelBtn.style.flex = '1';
        cancelBtn.innerHTML = `<span>${lt('Cancel')}</span>`;
        cancelBtn.onclick = function(e) {
            e.preventDefault();
        ...
        confirmBtn.href = '#';
        confirmBtn.className = 'kentools-btn primary';
    ...
    function ensureStyles() {
        if (!document.getElementById('kentools-spacing-styles')) {
            const style = document.createElement('style');
            style.id = 'kentools-spacing-styles';
            style.textContent = '.kentools-restart-button, .kentools-button, .kentools-icon-button{ margin-left:6px !important; }';
    ...
    function updateButtonTranslations() {
        try {
            ...
            const restartBtn = document.querySelector('.kentools-restart-button');
            if (restartBtn) {
            ...
            // Update Add via KenTools button
            const kentoolsBtn = document.querySelector('.kentools-button');
            if (kentoolsBtn) {
                const addViaText = lt('Add via KenTools');
                kentoolsBtn.title = addViaText;
                kentoolsBtn.setAttribute('data-tooltip-text', addViaText);
                const span = kentoolsBtn.querySelector('span');
                if (span) {
                    span.textContent = addViaText;
                }
            }
        } catch(err) {
            backendLog('KenTools: updateButtonTranslations error: ' + err);
        }
    }

    // Function to add the KenTools button
    function addKenToolsButton() {
        // Track current URL to detect page changes
        const currentUrl = window.location.href;
        if (window.__KenToolsLastUrl !== currentUrl) {
            // Page changed - reset button insertion flag and update translations
            window.__KenToolsLastUrl = currentUrl;
            window.__KenToolsButtonInserted = false;
            window.__KenToolsRestartInserted = false;
            window.__KenToolsIconInserted = false;
            window.__KenToolsPresenceCheckInFlight = false;
            window.__KenToolsPresenceCheckAppId = undefined;
        ...
        if (steamdbContainer) {
            ...
            const existingBtn = document.querySelector('.kentools-button');
            if (existingBtn) {
                ensureTranslationsLoaded(false).then(function() {
                    updateButtonTranslations();
                });
            ...
            if (existingBtn || window.__KenToolsButtonInserted) {
                if (!logState.existsOnce) { backendLog('KenTools button already exists, skipping'); logState.existsOnce = true; }
                // Even if KenTools exists, ensure Restart button is present and translations are updated
                return;
            }

            // Insert a Restart Steam button between Community Hub and our KenTools button
            try {
                if (!document.querySelector('.kentools-restart-button') && !window.__KenToolsRestartInserted) {
                    ensureStyles();
                    const referenceBtn = steamdbContainer.querySelector('a');
                    const restartBtn = document.createElement('a');
                    if (referenceBtn && referenceBtn.className) {
                        restartBtn.className = referenceBtn.className + ' kentools-restart-button';
                    } else {
                        restartBtn.className = 'btnv6_blue_hoverfade btn_medium kentools-restart-button';
                    }
                    restartBtn.href = '#';
                    const restartText = lt('Restart Steam');
                    restartBtn.title = restartText;
                    ...
                    restartBtn.addEventListener('click', function(e){
                        ...
                        try {
                            ...
                            closeSettingsOverlay();
                            showKenToolsConfirm('KenTools', lt('Restart Steam now?'),
                                function() { try { Millennium.callServerMethod('kentools', 'RestartSteam', { contentScriptQuery: '' }); } catch(_) {} },
                    ...
                    try {
                        if (!document.querySelector('.kentools-icon-button') && !window.__KenToolsIconInserted) {
                            const iconBtn = document.createElement('a');
                            if (referenceBtn && referenceBtn.className) {
                                iconBtn.className = referenceBtn.className + ' kentools-icon-button';
                            } else {
                                iconBtn.className = 'btnv6_blue_hoverfade btn_medium kentools-icon-button';
                            }
                            iconBtn.href = '#';
                            iconBtn.title = 'KenTools Helper';
                            iconBtn.setAttribute('data-tooltip-text', 'KenTools Helper');
                            // Normalize margins to match native buttons
                            try {
                                if (referenceBtn) {
                                    const cs = window.getComputedStyle(referenceBtn);
                            ...
                            try {
                                Millennium.callServerMethod('kentools', 'GetIconDataUrl', { contentScriptQuery: '' }).then(function(res){
                                    try {
                                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                        if (payload && payload.success && payload.dataUrl) {
                                            img.src = payload.dataUrl;
                                        } else {
                                            img.src = 'KenTools/kentools-icon.png';
                                        }
                                    } catch(_) { img.src = 'KenTools/kentools-icon.png'; }
                                });
                            } catch(_) {
                                img.src = 'KenTools/kentools-icon.png';
                            }
                            ...
                            window.__KenToolsIconInserted = true;
                            backendLog('Inserted Icon button');
                        }
                    } catch(_) {}
                    window.__KenToolsRestartInserted = true;
                    backendLog('Inserted Restart Steam button');
                }
            } catch(_) {}

            // If KenTools button already existed, stop here
            if (document.querySelector('.kentools-button') || window.__KenToolsButtonInserted) {
                return;
            }
            
            // Create the KenTools button modeled after existing SteamDB/PCGW buttons
            let referenceBtn = steamdbContainer.querySelector('a');
            const kentoolsButton = document.createElement('a');
            kentoolsButton.href = '#';
            // Copy classes from an existing button to match look-and-feel, but set our own label
            if (referenceBtn && referenceBtn.className) {
                kentoolsButton.className = referenceBtn.className + ' kentools-button';
            } else {
                kentoolsButton.className = 'btnv6_blue_hoverfade btn_medium kentools-button';
            }
            const span = document.createElement('span');
            const addViaText = lt('Add via KenTools');
            span.textContent = addViaText;
            kentoolsButton.appendChild(span);
            // Tooltip/title
            kentoolsButton.title = addViaText;
            kentoolsButton.setAttribute('data-tooltip-text', addViaText);
            // Normalize margins to match native buttons
            try {
                if (referenceBtn) {
                    const cs = window.getComputedStyle(referenceBtn);
                    kentoolsButton.style.marginLeft = cs.marginLeft;
                    kentoolsButton.style.marginRight = cs.marginRight;
                }
            } catch(_) {}
            
            // Local click handler suppressed; delegated handler manages actions
            kentoolsButton.addEventListener('click', function(e) {
                e.preventDefault();
                backendLog('KenTools button clicked (delegated handler will process)');
            });
            
            // Before inserting, ask backend if KenTools already exists for this appid
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                const appid = match ? parseInt(match[1], 10) : NaN;
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    // prevent multiple concurrent checks
                    if (window.__KenToolsPresenceCheckInFlight && window.__KenToolsPresenceCheckAppId === appid) {
                        return;
                    }
                    window.__KenToolsPresenceCheckInFlight = true;
                    window.__KenToolsPresenceCheckAppId = appid;
                    window.__KenToolsCurrentAppId = appid;
                    Millennium.callServerMethod('kentools', 'HasKenToolsForApp', { appid, contentScriptQuery: '' }).then(function(res){
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            if (payload && payload.success && payload.exists === true) {
                                backendLog('KenTools already present for this app; not inserting button');
                                window.__KenToolsPresenceCheckInFlight = false;
                                return; // do not insert
                            }
                            // Re-check in case another caller inserted during async
                            if (!document.querySelector('.kentools-button') && !window.__KenToolsButtonInserted) {
                                const restartExisting = steamdbContainer.querySelector('.kentools-restart-button');
                                if (restartExisting && restartExisting.after) {
                                    restartExisting.after(kentoolsButton);
                                } else if (referenceBtn && referenceBtn.after) {
                                    referenceBtn.after(kentoolsButton);
                                } else {
                                    steamdbContainer.appendChild(kentoolsButton);
                                }
                                window.__KenToolsButtonInserted = true;
                                backendLog('KenTools button inserted');
                            }
                            window.__KenToolsPresenceCheckInFlight = false;
                        } catch(_) {
                            if (!document.querySelector('.kentools-button') && !window.__KenToolsButtonInserted) {
            ...
            if (!logState.missingOnce) { backendLog('KenTools: steamdbContainer not found on this page'); logState.missingOnce = true; }
        }
    }
    ...
    function onFrontendReady() {
        ...
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('kentools', 'GetInitApisMessage', { contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.message) {
                            ...
                            if (isUpdateMsg) {
                                ...
                                showKenToolsConfirm('KenTools', msg, function() {
                                    // User clicked Confirm - restart Steam
                                    try { Millennium.callServerMethod('kentools', 'RestartSteam', { contentScriptQuery: '' }); } catch(_) {}
                                }, function() {
                ...
                try {
                    if (!sessionStorage.getItem('KenToolsLoadedAppsGate')) {
                        sessionStorage.setItem('KenToolsLoadedAppsGate', '1');
                        Millennium.callServerMethod('kentools', 'ReadLoadedApps', { contentScriptQuery: '' }).then(function(res){
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    document.addEventListener('click', function(evt) {
        const anchor = evt.target && (evt.target.closest ? evt.target.closest('.kentools-button') : null);
        if (anchor) {
            evt.preventDefault();
            backendLog('KenTools delegated click');
            // Use the same loading modal on delegated clicks
            if (!document.querySelector('.kentools-overlay')) {
                showTestPopup();
            }
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    if (runState.inProgress && runState.appid === appid) {
                        backendLog('KenTools: operation already in progress for this appid');
                        return;
                    }
                    runState.inProgress = true;
                    runState.appid = appid;
                    Millennium.callServerMethod('kentools', 'StartAddViaKenTools', { appid, contentScriptQuery: '' });
                    startPolling(appid);
                }
            } catch(_) {}
        }
    ...
    function startPolling(appid){
        ...
        const timer = setInterval(() => {
            ...
            try {
                Millennium.callServerMethod('kentools', 'GetAddViaKenToolsStatus', { appid, contentScriptQuery: '' }).then(function(res){
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        const st = payload && payload.state ? payload.state : {};
                        
                        // Try to find overlay (may or may not be visible)
                        const overlay = document.querySelector('.kentools-overlay');
                        const title = overlay ? overlay.querySelector('.kentools-title') : null;
                        const status = overlay ? overlay.querySelector('.kentools-status') : null;
                        const wrap = overlay ? overlay.querySelector('.kentools-progress-wrap') : null;
                        const percent = overlay ? overlay.querySelector('.kentools-percent') : null;
                        const bar = overlay ? overlay.querySelector('.kentools-progress-bar') : null;
                        
                        // Update UI if overlay is present
                        if (st.currentApi && title) title.textContent = lt('KenTools · {api}').replace('{api}', st.currentApi);
                        if (status) {
                        ...
                        if (st.status === 'downloading'){
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.kentools-cancel-btn') : null;
                        ...
                        if (st.status === 'done'){
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.kentools-cancel-btn') : null;
                            if (cancelBtn) cancelBtn.style.display = 'none';
                            const hideBtn = overlay ? overlay.querySelector('.kentools-hide-btn') : null;
                            if (hideBtn) hideBtn.innerHTML = '<span>' + lt('Close') + '</span>';
                            ...
                            const btnEl = document.querySelector('.kentools-button');
    ...
    setTimeout(addKenToolsButton, 1000);
    setTimeout(addKenToolsButton, 3000);
    ...
    function showLoadedAppsPopup(apps) {
        // Avoid duplicates
        if (document.querySelector('.kentools-loadedapps-overlay')) return;
        ensureKenToolsStyles();
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'kentools-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'kentools-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        const modal = document.createElement('div');
        modal.style.cssText = 'background:linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%);color:#fff;border:2px solid #dc2626;border-radius:8px;min-width:420px;max-width:640px;padding:28px 32px;box-shadow:0 20px 60px rgba(0,0,0,.8), 0 0 0 1px rgba(220,38,38,0.3);animation:slideUp 0.1s ease-out;';
        const title = document.createElement('div');
        title.style.cssText = 'font-size:24px;color:#fff;margin-bottom:20px;font-weight:700;text-shadow:0 2px 8px rgba(220,38,38,0.4);background:linear-gradient(135deg, #dc2626 0%, #f87171 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;text-align:center;';
        title.textContent = lt('KenTools · Added Games');
        const body = document.createElement('div');
        body.style.cssText = 'font-size:14px;line-height:1.8;margin-bottom:16px;max-height:320px;overflow:auto;padding:16px;border:1px solid rgba(220,38,38,0.3);border-radius:12px;background:rgba(11,20,30,0.6);';
        if (apps && apps.length) {
            ...
            apps.forEach(function(item){
                ...
                a.style.cssText = 'display:block;color:#c7d5e0;text-decoration:none;padding:10px 16px;margin-bottom:8px;background:rgba(220,38,38,0.08);border:1px solid rgba(220,38,38,0.2);border-radius:4px;transition:all 0.3s ease;';
                a.onmouseover = function() { this.style.background = 'rgba(220,38,38,0.2)'; this.style.borderColor = '#dc2626'; this.style.transform = 'translateX(4px)'; this.style.color = '#fff'; };
                a.onmouseout = function() { this.style.background = 'rgba(220,38,38,0.08)'; this.style.borderColor = 'rgba(220,38,38,0.2)'; this.style.transform = 'translateX(0)'; this.style.color = '#c7d5e0'; };
        ...
        dismissBtn.href = '#';
        dismissBtn.onclick = function(e){ e.preventDefault(); try { Millennium.callServerMethod('kentools', 'DismissLoadedApps', { contentScriptQuery: '' }); } catch(_) {} try { sessionStorage.setItem('KenToolsLoadedAppsShown', '1'); } catch(_) {} overlay.remove(); };
